...
Runs the three variants reported in Table 6 of the IEEE ICDM 2026 paper
on the same final splits used in the main benchmark tables:

  V1  Residual-only      : no text branch, no KPI contrastive, no uncertainty
  V2  +Text fusion       : full text branch, no KPI contrastive
  V3  KAC (ours)         : full text + uncertainty-weighted KPI contrastive

V3 is the full KAC model and reproduces the headline KAC numbers in
Tables 2 and 4. Each variant is trained with five seeds
(42, 123, 456, 789, 1337).
...
Usage
-----
This module is the ablation driver. The recommended entry point is the
unified CLI, which wraps it and centralizes outputs::

    python main.py ablation --dataset telecomts --variants V1 V2 V3
    python main.py ablation --dataset spotlight

It can also be invoked directly; in that case it resolves the NPZ inputs
and the Chronos-2 residual cache relative to the current working
directory (the layout produced by the ``scripts/`` downloaders)::

    python -m experiments._shared.kac_ablation --dataset TelecomTS

The ``Production`` (ProdTrace-SA) configuration is included so the
pipeline can be inspected and rerun by collaborators with data access,
but it is intentionally not runnable from a clean checkout because the
Production data is Nokia-internal and not redistributed. The public
ablation table (Table 6) reports TelecomTS and SpotLight only.
See ``docs/data_availability.md`` for the full statement.

Notes
-----
* Reuses the exact model definition, loss functions and training loop
  from the headline KAC notebooks so V3 is identical to the headline
...
def kpi_contrastive_loss(z_text_kpi, z_resid_kpi, width_raw,
    ...
    are uniform. The full KAC model (V3) uses uncertainty weighting.
    """
...
VARIANTS: List[VariantSpec] = [
    ...
    VariantSpec("V3", "KAC (uncertainty-weighted KPI contrastive, ours)", True, True, True),
]
...
def run_ablation(
    ...
    ----------
    dataset
        One of ``"TelecomTS"``, ``"SpotLight"``, ``"Production"``. When
        invoked directly the function resolves ``data/...`` and
        ``data/features_cache/`` relative to the current working directory
        (the splits + Chronos-2 residual cache produced by ``scripts/``).
        Prefer ``python main.py ablation`` which resolves these paths and
        centralizes outputs automatically.
    seeds
        List of integer seeds. Defaults to ``[42, 123, 456, 789, 1337]``.
    variants
        List of variant codes to run. Defaults to ``["V1", "V2", "V3"]``.
    out
...
def main():
    ...
    ap.add_argument("--variants", type=str, nargs="*",
                    ...
                    help="Variant codes to run, e.g. V1 V2 V3")
