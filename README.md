...
Table/figure numbers below follow the submitted paper: **Figure 1** is the KAC
overview; **Table 1** dataset stats, **Table 2** TelecomTS, **Table 3** TelecomTS
alert budget, **Table 4** zero-shot LLM, **Table 5** SpotLight, **Table 6**
ProdTrace-SA, **Table 7** paired significance, **Table 8** ablation, **Table 9**
operational cost.

| Command | Paper artefact | What it produces |
|---|---|---|
| `main.py kac --dataset telecomts --scenario balanced` | **Table 2** (KAC row, balanced) | Multi-seed KAC headline numbers on the balanced TelecomTS split |
| `main.py kac --dataset telecomts --scenario imbalanced` | **Table 2** (KAC row, imbalanced) | Same pipeline on the imbalanced stress split |
| `main.py kac --dataset spotlight` | **Table 5** (KAC row) | KAC headline numbers on SpotLight Open RAN |
| `main.py sota --dataset telecomts` | **Table 2** (DCdetector, D3R, MEMTO, ModernTCN, TimesNet) | Trained deep MTSAD baselines |
| `main.py sota --dataset spotlight` | **Table 5** (deep MTSAD block) | Deep MTSAD baselines on SpotLight |
...
| `main.py foundation --dataset spotlight --foundation-model {moment,toto,mantis}` | **Table 5** (foundation block) | TSFM baselines on SpotLight |
| `main.py spotlight-baseline --dataset telecomts` | **Table 2** (SpotLight-pipeline row) | SpotLight JVGAN/MRPI pipeline baseline |
| `main.py llm --dataset telecomts --provider {gpt,gemini,claude}` | **Table 4** (TelecomTS rows) | Zero-shot LLM scoring |
| `main.py llm --dataset spotlight --provider {gpt,gemini,claude}` | **Table 4** (SpotLight rows) | Zero-shot LLM scoring |
| `main.py ablation --dataset telecomts --variants V1 V2 V3` | **Table 8** (TelecomTS) | V1 → V3 KAC component ablation (5 seeds) |
| `main.py ablation --dataset spotlight --variants V1 V2 V3` | **Table 8** (SpotLight) | V1 → V3 KAC component ablation (5 seeds) |

Remaining paper artefacts not tied to a single training command:

| Paper artefact | How it is produced |
|---|---|
| **Figure 1** (KAC overview) | Architecture diagram (`figures/fig2.pdf`); not generated by an experiment |
| **Table 1** (dataset characteristics) | Descriptive statistics of the processed splits |
| **Table 3** (TelecomTS alert budget) | Fixed-FPR (2.5%) recalibration of KAC / QR-TAN / Chronos-2+LR scores |
| **Table 6** (ProdTrace-SA) | `main.py kac --dataset production` (+ baselines); Nokia-internal data, not redistributable |
| **Table 7** (paired ΔF1 significance) | Bootstrap CIs over the per-seed `metrics.csv` outputs |
| **Table 9** (operational cost) | `python -m deployment.benchmark_latency` (params + CPU latency) |
...
# Component ablation V1/V2/V3 (paper Table 8).
